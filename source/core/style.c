...
// Lua state is stored globally while processing a stylesheet table so that
// parse_property (called from OBJ_ApplyClass) can resolve Lua values
...
// Apply all active style rules to an object (and optionally its children)
void
OBJ_ApplyStyles(lpObject_t object, bool_t recursive)
{
  ...
  if (sc) {
    ...
    FOR_EACH_LIST(struct style_class, cls, sc->classes) {
