...
void OBJ_post(lua_State* L, struct Object *self, lpcString_t message)
{
  ...
  if (luaL_testudata(L, 3, argtype)) {
    SV_PostMessageData(self, message, 0, lua_touserdata(L, 3), lua_rawlen(L, 3));
    ...
    SV_PostMessageData(self, message, 0, lua_touserdata(L, -1), lua_rawlen(L, -1)); // Write the constructed message args to the event data buffer and post the message
