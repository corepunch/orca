...
static int set_prop(lua_State* L) { OBJ_SetProperty(L, luaX_checkObject(L, 1), luaL_checkstring(L, 2)); return 0; }

static void _ParseArguments(lua_State* L, struct Object *hobj) {
  if (lua_type(L, 2) == LUA_TSTRING) {
    lpcString_t arg = luaL_checkstring(L, 2);
    if (*arg == '#') {
      WITH(char, dup, strdup(++arg), free) {
        OBJ_SetName(hobj, strtok(dup, "."));
        while ((arg = strtok(NULL, "."))) _SendMessage(hobj, StyleController, AddClass, arg);
        lua_remove(L, 2);
      }
    } else if (*arg == '.') {
      WITH(char, dup, strdup(++arg), free) {
  ...
  if (lua_type(L, 2) == LUA_TTABLE) {
    ...
    while (lua_next(L, 2)) {
      ...
      lua_pushcfunction(L, set_prop); luaX_pushObject(L, hobj);
      lua_pushvalue(L, key_idx); lua_pushvalue(L, val_idx);
      lua_call(L, 3, 0); lua_pop(L, 1);
    }
    lua_remove(L, 2); _SendMessage(hobj, Object, Start);
  }
  if (lua_type(L, 2) == LUA_TFUNCTION) { OBJ_Rebuild(L, hobj); lua_remove(L, 2); }
  if (lua_type(L, 2) == LUA_TSTRING) { OBJ_SetTextContent(hobj, luaL_checkstring(L, 2)); lua_remove(L, 2); }
  if (lua_isnil(L, 2)) { lua_remove(L, 2); return; }
  if (lua_gettop(L) > 1) Con_Error("Can't parse argument of type %d, stack size %d", lua_type(L, 2), lua_gettop(L));
}
...
int OBJ_CreateFromLuaState(lua_State *L) {
  ...
  if (*ctx) OBJ_AddChild(*ctx, pobj);
  _ParseArguments(L, pobj);
  lua_getfield(L, 1, "apply");
  if (lua_type(L, -1) == LUA_TFUNCTION) {
    luaX_pushObject(L, pobj);
    if (lua_pcall(L, 1, 1, 0) != LUA_OK) Con_Error("Can't execute apply()");
    else _SendMessage(pobj, StyleController, AddClasses, luaL_checkstring(L, -1));
  }
  lua_pop(L, 1); luaX_pushObject(L, pobj); OBJ_ReleaseRef(pobj);
  return 1;
}
...
void OBJ_Rebuild(lua_State* L, struct Object *self) {
  ...
  *((struct Object **)lua_getextraspace(co)) = self;
  ...
  lua_pushcfunction(co, f_rebuild); luaX_pushObject(co, self);
