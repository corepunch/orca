...
//      WI_PostMessageW(msg->target, kEventWindowPaint, msg->wParam, NULL);
    ...
    case kEventResumeCoroutine:
      // msg->wParam contains the number of arguments (not including function)
      // msg->target is the coroutine thread to resume
      // msg->lParam is the registry reference for cleanup
      switch (lua_resume(msg->target, L, msg->wParam, NULL)) {
        case LUA_OK:
          WI_PostMessageW(msg->target, kEventStopCoroutine, msg->wParam, msg->lParam);
          break;
        case LUA_YIELD:
          // When resuming after yield, pass 0 arguments
