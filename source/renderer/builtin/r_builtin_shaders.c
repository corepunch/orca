...
struct shader_desc shader_charset= {
  ...
  "  float t = tex.a;\n"
  "  t = mix(t,1.0-t,float(all(equal(u_selectedItem,chr.ba))));\n"
  "  vec2 curs = abs(u_cursorPos-v_texcoord0*u_textureSize);\n"
  "  t = mix(t,1.0-t,float(curs.x<0.5&&curs.y<0.5&&fr.x<0.25));\n"
...
//    { "v_texcoord0", UT_FLOAT_VEC2 },
  ...
  "  pos.x = mix((pos.x + 0.5) * koeff - 0.5, (pos.x - 0.5) * koeff + 0.5, float(pos.x > 0.0));\n"
  "  pos *= rectSize.xyy;\n"
