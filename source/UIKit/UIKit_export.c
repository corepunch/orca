...
static int f_new_BorderRadiusShorthand(lua_State *L) {
	...
	if (lua_istable(L, 1)) {
		lua_pop(L, (lua_getfield(L, 1, "TopLeftRadius"), self->TopLeftRadius = lua_tonumber(L, -1), 1));
		lua_pop(L, (lua_getfield(L, 1, "TopRightRadius"), self->TopRightRadius = lua_tonumber(L, -1), 1));
		lua_pop(L, (lua_getfield(L, 1, "BottomRightRadius"), self->BottomRightRadius = lua_tonumber(L, -1), 1));
		lua_pop(L, (lua_getfield(L, 1, "BottomLeftRadius"), self->BottomLeftRadius = lua_tonumber(L, -1), 1));
	} else {
...
static int f_new_FontShorthand(lua_State *L) {
	...
	if (lua_gettop(L) == 1) return 1;
	if (lua_istable(L, 1)) {
		lua_pop(L, (lua_getfield(L, 1, "Weight"), self->Weight = lua_isnil(L, -1) ? 0 : luaX_checkFontWeight(L, -1), 1));
		lua_pop(L, (lua_getfield(L, 1, "Style"), self->Style = lua_isnil(L, -1) ? 0 : luaX_checkFontStyle(L, -1), 1));
		lua_pop(L, (lua_getfield(L, 1, "Size"), self->Size = lua_tonumber(L, -1), 1));
	} else {
...
static int f_new_ShadowShorthand(lua_State *L) {
	...
	if (lua_gettop(L) == 1) return 1;
	if (lua_istable(L, 1)) {
		lua_pop(L, (lua_getfield(L, 1, "BlurRadius"), self->BlurRadius = lua_tonumber(L, -1), 1));
		lua_pop(L, (lua_getfield(L, 1, "SpreadRadius"), self->SpreadRadius = lua_tonumber(L, -1), 1));
	} else {
...
static int f_new_OverflowShorthand(lua_State *L) {
	...
	if (lua_gettop(L) == 1) return 1;
	if (lua_istable(L, 1)) {
		lua_pop(L, (lua_getfield(L, 1, "x"), self->x = lua_isnil(L, -1) ? 0 : luaX_checkOverflow(L, -1), 1));
		lua_pop(L, (lua_getfield(L, 1, "y"), self->y = lua_isnil(L, -1) ? 0 : luaX_checkOverflow(L, -1), 1));
	} else {
...
static int f_new_SizeAxisShorthand(lua_State *L) {
	...
	if (lua_gettop(L) == 1) return 1;
	if (lua_istable(L, 1)) {
		lua_pop(L, (lua_getfield(L, 1, "Requested"), self->Requested = lua_tonumber(L, -1), 1));
		lua_pop(L, (lua_getfield(L, 1, "Desired"), self->Desired = lua_tonumber(L, -1), 1));
		lua_pop(L, (lua_getfield(L, 1, "Min"), self->Min = lua_tonumber(L, -1), 1));
		lua_pop(L, (lua_getfield(L, 1, "Actual"), self->Actual = lua_tonumber(L, -1), 1));
		lua_pop(L, (lua_getfield(L, 1, "Scroll"), self->Scroll = lua_tonumber(L, -1), 1));
	} else {
...
static int f_new_NavigateToPageArguments(lua_State *L) {
	...
	if (lua_istable(L, 1)) {
		...
		lua_pop(L, (lua_getfield(L, 1, "TransitionType"), self->TransitionType = lua_isnil(L, -1) ? 0 : luaX_checkTransitionType(L, -1), 1));
	} else {
