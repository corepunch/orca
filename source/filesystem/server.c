...
static int print_classes(lpcClassDesc_t cd, LPSTR s, int n) {
  ...
  n -= w;
  for (uint32_t const *p = cd->ParentClasses; *p && n > 1; p++)
    w += print_classes(OBJ_FindClassW(*p), s + w, n - w);
  return w;
}
...
LRESULT filesystem_handle_event(lua_State *L, struct WI_Message *msg) {
  if (msg->message == ID_ReadCommands) {
    ...
    WI_PostMessageW(msg->target, ID_ReadCommands, 0, NULL);
    return TRUE;
  } else {
    return FALSE;
  }
