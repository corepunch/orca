...
Same as `REGISTER_CLASS` but sets `.ClassFlags = TRUE`. Component components **cannot be instantiated standalone**; they must be added to an existing object with `OBJ_AddComponent`. Example: `AnimationPlayer`.

The `` attribute in the XML `<class>` element triggers this macro instead of `REGISTER_CLASS`.
