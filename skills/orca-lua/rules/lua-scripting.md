...
The `*_export.c` files generated by `make modules` wire every `.cgen`-declared struct, interface, and class into Lua. Understanding the generated binding pattern helps when debugging.
...
| Main loop | Lua `while true do … end` calls `axPeekMessage`; ASYNCIFY yields to the browser on WebGL |
