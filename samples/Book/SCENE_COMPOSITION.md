...
3. **Render both the establishing view and the layout immediately.** From
   `samples/Book`:
   ```sh
   make render ROOM=workshop   # perspective establishing shot
   make layout ROOM=workshop   # top-down floor plan (scener --layout)
   ```
   The layout is generated by Scener's built-in overhead mode, not a scene
   camera; there is no `LayoutPlan` camera in the `.blks` file.
   Run both before touching anything else. The plan catches positional errors
...
Confirm both renders before proceeding to secondary objects or detail.
...
Wondertown Workshop uses a **Romantic toy-maker Gothic** vocabulary:

| Element | Rule |
|---|---|
| All openings | Roman (semicircular) arch — `bool-negative-arch` |
| Main door | Tall arch, ~240 cm wide, ~460 cm tall |
| Windows | Narrower tall arches, sill ~120–150 cm off floor |
| Pet door | Small arch at base of the main door arch |
| Accent windows | Circular / rose / oculus windows above major arches (stub as small arch until a round cutter is available) |
| Wall surfaces | Smooth plaster, teal-painted timber accents at final-art stage |
| Ceiling | Exposed wooden beams |

The reference image is `Work/references/` concept art showing the character Pip in the workshop. Carry the Roman arch consistently: if an opening is rectangular in the blockout, it is wrong and must be replaced with `bool-negative-arch`.
...
Named anchors are self-closing `<group>` elements. They carry no child geometry
at blockout stage:

```xml
<group name="KEY-HOOK"   pos="-576 250 360"/>
<group name="CLOCK-FACE" pos="-576 500 390"/>
<group name="OIL-CAN"    pos="-150  90  20"/>
```

Add visible geometry to an anchor only when it is a major form that must read
as a silhouette in the establishing shot (e.g. the workbench itself). Wall hooks,
small props, and door hardware do not need blockout geometry.

### Window and door openings

Every opening — door, window, pet door — uses `bool-negative-arch` (Roman
arch). This is the architectural style of the workshop: semicircular arched
tops on all cutouts, no rectangular holes. No frame, no glass, no mullions at
blockout stage; those are final-art details.

```xml
<!-- tall arched window on east wall -->
<bool-negative-arch pos="600 820 265" rot="0 0 90" width="130" height="230" depth="50"/>
<!-- small pet-door arch at base of main north arch -->
<bool-negative-arch pos="-170 1000 50"              width=" 80" height="100" depth="50"/>
```

The reference for this style is `Images/Generated image 1.png` / the concept
art: Roman arches on every opening, circular accent windows (rose/oculus) as
secondary accents above larger arches. Circular windows are noted as a style
element but are not yet supported by a blockout primitive; stub them as small
arches until a round cutter is available.

Add frame geometry only after the opening position and size are locked by a
render review.
...
After **any** change to a `.blks` file, always run both:

```sh
make render ROOM=workshop   # perspective camera(s)
make layout ROOM=workshop   # top-down floor plan
```

Inspect the layout first — it catches object placement errors (floating
furniture, wall gaps, missed exits) that the perspective shot can hide.
Then inspect the perspective render for composition and occlusion.

After moving cameras or named anchors, also regenerate metadata:
...
The exported metadata and JPEGs must come from the same `.blks` revision.
