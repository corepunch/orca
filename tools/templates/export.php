...
#include "<?= substr($controller->source, 0, -4) ?>.h"
...
ORCA_API int luaopen_orca_<?= $controller->getModuleName() ?>(lua_State *L) {
	luaL_newlib(L, ((luaL_Reg[]) { { NULL, NULL } }));
	<?php if ($controller->on_luaopen): ?>
	void <?= $controller->on_luaopen ?>(lua_State *L);
	<?= $controller->on_luaopen ?>(L);
	<?php endif ?>
	// Structs
