...
int f_<?= $prefix.$name ?>(lua_State *L) {
	<?php $index = 1 ?>
	...
	return <?= $prefix.$name ?>(<?= implode(", ", array_keys($func->getArgs())) ?>);
	<?php elseif ($func->returns): ?>
	<?= $func->returns ?> result_ = <?= $prefix.$name ?>(<?= implode(", ", array_keys($func->getArgs())) ?>);
	<?= $func->returns->get('push', 'result_') ?>;
	return 1;
	<?php else: ?>
	<?= $prefix.$name ?>(<?= implode(", ", array_keys($func->getArgs())) ?> <?= $func->getReturnType() != "void" ? ", " : "" ?>);
	return 0;
	<?php endif ?>
}
<?php endforeach ?>