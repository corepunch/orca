...
```
plugins/UIKit/
├── UIKit.cgen           # API definition — source of truth
...
The hand-written files are `UIKit.c` and the per-component `*.c` files. Everything else is generated by the C codegen toolchain from `UIKit.cgen`.

### Plugin loading flow

1. At startup the engine scans the plugins directory for `*.so` files.
2. For each library it calls `luaopen_orca_<Name>(L)` — the function generated in `*_export.c`.
3. That generated function calls the `on-luaopen` callback declared in the `.cgen` file (e.g. `on_ui_module_registered`).
...
2. Run `make modules` to generate the bindings.
